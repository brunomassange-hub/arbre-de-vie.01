...
export default function ClinicalTagSelector({ value = [], onChange }) {
  const [search, setSearch] = useState("");
  const [expandedLists, setExpandedLists] = useState({});

  const filteredLists = useMemo(() => {
    return CLINICAL_LISTS.map(list => ({
      ...list,
                  ...
                  {list.items.map(item => {
                    ...
                    return (
                      <button key={item.id} type="button" title={item.description}
                        ...
                        className={`px-1.5 py-0.5 rounded-full text-[9px] border transition ${sel ? `${LIST_COLORS[list.id]} text-white border-transparent` : "bg-white/60 text-[#8d6e63] border-[#e0d6c8] hover:bg-white"}`}>
                        {item.label}
                      </button>
                    );
                  })}
